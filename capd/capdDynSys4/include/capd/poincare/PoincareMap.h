...
class PoincareMap : public BasicPoincareMap<SolverT, FunT>
{
public:
  ...
  typedef FunT FunctionType;    ///< type of function \f$ s: R^n \to R \f$ that represents Poincare section.
  ...
  typedef typename BasicPoincareMap<Solver,FunT>::CrossingDirection CrossingDirection; ///< section crossing direction; possible values are PlusMinus, Both(default) and MinusPlus.
...
protected:
  ...
  void saveJacEnclosure(capd::dynset::CnSet<MatrixType,DEGREE>&);
  ...
  void updateJacEnclosure(C1Set &S);
  void updateJacEnclosure(C2Set &S);
  template<capd::dynset::__size_type DEGREE>
  void updateJacEnclosure(capd::dynset::CnSet<MatrixType,DEGREE>&);
  ...
  void* jet;					          ///< stores derivative of the flow (for C^n sets only)
