      ...
      typename DS::MatrixType& Dpsi)
{
  ...
  for(k=p-1;k>=0;k--)
  {
    for(i=0;i<dimension;++i)
    {
      psi[i] = psi[i]*h + newton(p+q-k,q)*ds.centerCoefficient(i,k)/denominator;
      for(j=0;j<dimension;++j)
        Dpsi(i+1,j+1) = Dpsi(i+1,j+1)*h + newton(p+q-k,q)*ds.coefficient(i,j,k)/denominator;
    }
  }
} // end of computePsi
