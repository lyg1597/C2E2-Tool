...
namespace Sub
{
  ...
  inline void evalHomogenousPolynomial(const unsigned degree, const T* left, const T* right, T* result, const unsigned dim, const unsigned order)
  {
    if(degree)
    {
      ...
      right += s;
      result +=s;
      for(unsigned derNo=0;derNo<newton(dim-1+degree,degree);++derNo,left+=order,right+=order,result+=order)
        *result = *left - *right;
    } else
      *result = *left - *right;
  }
...
namespace ConstMinusVar
{
  ...
  inline void evalHomogenousPolynomial(const unsigned degree, const T* left, const T* right, T* result, const unsigned dim, const unsigned order)
  {
    if(degree)
    {
      ...
      right += s;
      result +=s;
      for(unsigned derNo=0;derNo<newton(dim-1+degree,degree);++derNo,right+=order,result+=order)
        *result = -(*right);
    } else
      *result = *left - *right;
  }
