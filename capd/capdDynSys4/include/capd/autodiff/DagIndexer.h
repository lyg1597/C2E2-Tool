...
*/
  ...
  JetSize jetSize()          const { return JetSize(newton(m_domainDimension+m_degree,m_degree)); }
...
inline typename DagIndexer<T>::iterator DagIndexer<T>::begin(size_type i, size_type d){
  return d==0
         ? this->begin(i)
         : this->begin(i) + (this->getOrder()+1)*newton(this->m_domainDimension+d-1,d-1);
}

template<class T>
inline typename DagIndexer<T>::iterator DagIndexer<T>::end(size_type i, size_type d){
  return this->begin(i) + (this->getOrder()+1)*newton(this->m_domainDimension+d,d);
}
