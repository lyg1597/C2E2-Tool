 ...
 * Sections par and time are obsolete.
 *
 ...
 * \endcode
 *
 * The map can be parsed from a C-like routine. One has to write a global function that defines map and send it to the constructor.
 * The function that defines map must have the following header:
 *
 * \code
 * #include "capd/capdlib.h"
 ...
 *
 * The directed acyclic graph (DAG) that encodes map is stored as well indexed array. Thus processing of it is fast.
