...
/// @param [out] knownNodes - this is a temporary variable for rrecursive calls of this function. It contains an array of already recognized subexpression. Used for optimization of DAG.
int parseExpression(
...
/*
  ...
  if(Parser::searchForFunction("atan", eq, params)){
    ...
    m_nodes[eq]= *N = addBinaryNode<capd::diffAlgebra::AtanNode>(params, help);
    return;
  }
  // arc sin
    ...
    if(dag[i].op == NODE_POW)
    {
      ...
      if(dag[right].op == capd::autodiff::NODE_CONST)
      {
        if(dag[right].val == 2.){
          ...
          dag[i].op = NODE_INTEGER_POW;
          optimizeUnivariateFunction(dag,left,i,NODE_INTEGER_POW,NODE_INTEGER_POW_CONST,NODE_INTEGER_POW_TIME,NODE_INTEGER_POW_FUNTIME);
        }
        else if((int)(2*dag[right].val) == 2*dag[right].val)
          dag[i].op = NODE_HALF_INTEGER_POW;
          optimizeUnivariateFunction(dag,left,i,NODE_HALF_INTEGER_POW,NODE_HALF_INTEGER_POW_CONST,NODE_HALF_INTEGER_POW_TIME,NODE_HALF_INTEGER_POW_FUNTIME);
      }
    }
  }
}
