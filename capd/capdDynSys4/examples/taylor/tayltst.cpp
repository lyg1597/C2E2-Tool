...
void initGraph(double step, int order)
{
   ...
   rootFrame.Clear();
   axes(fr);
   rootFrame << "Test of set arithemtic using class IMap and ITaylor.\n";
...
void C012Tests()
{
   ...
   IC2Taylor DS(f,order);
   DS.setStep(step);
// C^0 - test
   IVector X(DInterval(0.1), DInterval(1.5259617305037), DInterval(0.2));
   C2Rect2Set S1(X);
         ...
         for(c=0;c<DIMENSION;c++)
           {
            for(k=0;k<DIMENSION;k++)
            {
               D2Q[i][j][c] += power(-1,i+k) * DP2[i][k] * (j==c? 2.*S1(k,j,c):S1(k,j,c));
               for(ss=0;ss<DIMENSION;ss++)
               {
                  D2Q[i][j][c] += power(-1,i+k+ss+1) * (k==ss? 2.*S2(i,k,ss):S2(i,k,ss)) * DP1[ss][c]*DP1[k][j];
               }
            }
...
int main(int, char *[])
{
   ...
   try
   {
      ...
      computeRigorousTrajectory(intv2,T,RED,"C0Intv2Set");
      computeRigorousTrajectory(rect,T,MAGENTA,"C0RectSet");
      computeRigorousTrajectory(rect2,T,GREEN,"C0Rect2Set");
      computeRigorousTrajectory(ho2,T,VIOLET,"C0HORect2Set");
