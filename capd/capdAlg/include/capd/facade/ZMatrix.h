...
public:
  ...
  //constructors
  ZMatrix(){}
  ZMatrix(size_type rows,size_type cols)
    : m_M(rows,cols) 
  {}
  
  ZMatrix(size_type rows,size_type cols, const ScalarType data[])
    : m_M(rows,cols,data)
  {}
  ...
  ZMatrix(size_type rows,size_type cols,bool b)
    : m_M(rows,cols,b)
  {}
  ...
  const int& operator()(size_type i,size_type j) const{
    return m_M(i,j);
  }
  int* at(size_type i,size_type j){
    return m_M.at(i,j);
  }
  const int* at(size_type i,size_type j) const{
    return m_M.at(i,j);
  }

  //operations on matrices
  static ZMatrix Identity(size_type dimension){
