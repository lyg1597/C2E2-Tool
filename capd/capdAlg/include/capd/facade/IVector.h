...
// in the future we include here a facade for intervals
...
public:
  ...
  const static size_type csDim = BaseVector::csDim;
  ...
  IVector(size_type dim,DInterval tab[] )
    : m_v(dim,tab)
  {}
  ...
  DInterval& operator[](size_type i){
    return m_v[i];
  }

  const DInterval& operator[](size_type i) const{
    return m_v[i];
  }
  ...
  void* operator new[](size_t sizeOfData,size_type newSize)
  {
...
inline DInterval maxDiam(const IVector& u){
  return capd::vectalg::maxDiam(u);
}
