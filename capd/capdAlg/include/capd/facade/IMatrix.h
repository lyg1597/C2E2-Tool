...
public:
  ...
  //constructors
  IMatrix(){}
  IMatrix(size_type rows,size_type cols)
    : m_M(rows,cols)
  {}

  IMatrix(size_type rows,size_type cols, const ScalarType data[])
    : m_M(rows,cols,data)
  {}
  ...
  IMatrix(size_type rows,size_type cols,bool b)
    : m_M(rows,cols,b)
  {}
  ...
  MatrixIterator< BaseMatrix > beginOfRow(size_type i){
    return m_M.beginOfRow(i);
  }
  MatrixIterator< BaseMatrix > beginOfColumn(size_type j){
    return m_M.beginOfColumn(j);
  }
  MatrixIterator< BaseMatrix > endOfRow(size_type i){
    return m_M.endOfRow(i);
  }
  MatrixIterator< BaseMatrix > endOfColumn(size_type j){
  ...
  const_MatrixIterator< BaseMatrix > beginOfRow(size_type i) const{
    return m_M.beginOfRow(i);
  }
  const_MatrixIterator< BaseMatrix > beginOfColumn(size_type j) const{
    return m_M.beginOfColumn(j);
  }
  const_MatrixIterator< BaseMatrix > endOfRow(size_type i) const{
    return m_M.endOfRow(i);
  }
  const_MatrixIterator< BaseMatrix > endOfColumn(size_type j) const{
    return m_M.endOfColumn(j);
  }
  ...
  //indexing
  RefRowVectorType operator[](size_type i) const{
    return m_M[i];
  }
  RefRowVectorType operator()(size_type i) const{
    return m_M(i);
  }
  DInterval& operator()(size_type i,size_type j){
    return m_M(i,j);
  }
  const DInterval& operator()(size_type i,size_type j) const{
    return m_M(i,j);
  }
  DInterval* at(size_type i,size_type j){
    return m_M.at(i,j);
  }
  const DInterval* at(size_type i,size_type j) const{
    return m_M.at(i,j);
  }

  //operations on matrices
  static IMatrix Identity(size_type dimension){
  ...
  RefRowVectorType row(size_type i) const{
    return m_M.row(i);
  }
  RefColumnVectorType column(size_type j) const{
    return m_M.column(j);
  }
  void Transpose(){
    m_M.Transpose();
  ...
  void* operator new[](size_t sizeOfData,size_type rows, size_type cols)
  {
    BaseMatrix::setDefaultDimension(std::pair<size_type,size_type>(rows,cols));
    return ::operator new[](sizeOfData);
  }
