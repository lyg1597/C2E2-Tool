...
def createCDFfunction(delete_element):
    ...
    ComputeLDFstring += "   tempdata = []\n"
    ComputeLDFstring += "   tempdata = Simulation_data[:,notbloating[i]]\n"
    ComputeLDFstring += "   for j in range (0,len(Simulation_data),2):\n"
    ComputeLDFstring += "       tempdata[j]= tempdata[j]- deltacopy[notbloating[i]-1]\n"
    ComputeLDFstring += "       tempdata[j+1] = tempdata[j+1] + deltacopy[notbloating[i]-1]\n"
    ComputeLDFstring += "   temp = (notbloating[i],tempdata)\n"
